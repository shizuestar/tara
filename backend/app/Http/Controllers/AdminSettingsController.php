   ...
   class AdminSettingsController extends Controller
   {
       public function index()
       {
           ...
           return view('administrator.admin.settings.index', compact(
               'settings', 'roles', 'permissions', 'selected_role_id', 'selected_role', 'selected_role_permissions',
       ...
       public function updatePermissions(Request $request)
       {
           ...
           return redirect()->route('admin.settings.index', ['role_id' => $validated['role_id']])->with('success', 'Hak akses diperbarui.');
       }
       ...
       {
           $validated = $request->validate(['type' => 'in:database,media']);
           // Logika untuk membuat backup (misalnya, menggunakan package spatie/laravel-backup)
           $filePath = "backups/{$type}_" . now()->format('YmdHis') . ($type == 'database' ? '.sql' : '.zip');
           // Simulasi penyimpanan file
           Backup::create(['type' => $type, 'file_path' => $filePath]);
           return redirect()->route('admin.settings.index')->with('success', 'Backup dibuat.');
       }

       public function restoreBackup(Request $request)
       {
           $validated = $request->validate(['backup_file' => 'required|file|mimes:sql,zip']);
           $filePath = $request->file('backup_file')->store('backups', 'public');
           // Logika untuk restore (implementasi bergantung pada sistem)
           return redirect()->route('admin.settings.index')->with('success', 'Restore dimulai.');
       }
       ...
       {
           $validated = $request->validate(['schedule' => 'required|in:disabled,daily,weekly,monthly']);
           // Simpan pengaturan jadwal di config atau database
