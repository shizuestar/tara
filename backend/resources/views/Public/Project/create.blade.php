<x-layout>
    ...
    <section class="pt-20 pb-12 section-gradient">
        <div class="container">
            ...
            <form action="{{ route('proyek.store') }}" method="POST" enctype="multipart/form-data" class="max-w-2xl mx-auto">
                @csrf
                <div class="space-y-6">
                    <div>
                    ...
                    <div>
                        ...
                        <textarea name="description" id="description"
                                  class="w-full px-3 py-2 border border-gray-200 rounded text-sm focus:outline-none focus:ring-2 focus:ring-yellow-400"
                                  rows="5" placeholder="Jelaskan tentang proyek Anda" required>{{ old('description') }}</textarea>
                        @error('description')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <textarea name="collaboration_goals" id="collaboration_goals"
                                  class="w-full px-3 py-2 border border-gray-200 rounded text-sm focus:outline-none focus:ring-2 focus:ring-yellow-400"
                                  rows="5" placeholder="Apa tujuan kolaborasi proyek ini?" required>{{ old('collaboration_goals') }}</textarea>
                    ...
                    <div>
                        ...
                        <select name="status" id="status"
                                class="w-full px-3 py-2 border border-gray-200 rounded text-sm focus:outline-none focus:ring-2 focus:ring-yellow-400"
                                required>
                            <option value="open" {{ old('status') == 'open' ? 'selected' : '' }}>Buka Kolaborasi</option>
                            <option value="closed" {{ old('status') == 'closed' ? 'selected' : '' }}>Kolaborasi Ditutup</option>
                        </select>
                        @error('status')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    ...
                    <div class="flex gap-4">
                        <label class="flex items-center">
                            <input type="checkbox" name="populer" {{ old('populer') ? 'checked' : '' }}
                                   class="mr-2 focus:ring-yellow-400" />
                            Tandai sebagai Populer
                        </label>
                        <label class="flex items-center">
                            <input type="checkbox" name="baru" {{ old('baru') ? 'checked' : '' }}
                                   class="mr-2 focus:ring-yellow-400" />
                            Tandai sebagai Baru
                        </label>
                    </div>
                    <div class="flex justify-end gap-4">
                        <a href="{{ route('proyek.index') }}" class="action-btn">Batal</a>
