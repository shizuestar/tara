<x-layout>
                    ...
                    <div id="bookmark-posts" class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        ...
                        @forelse ($filteredBookmarks as $bookmark)
                            @php
                                ...
                                $imagePath = $post->cover_image ?? $post->image ?? 'https://picsum.photos/600/400';
                                // Hanya render item yang diperlukan oleh Blade saat page load, sisanya ditangani JS
                            @endphp
                            {{-- Di sini Anda bisa meninggalkan @forelse kosong dan sepenuhnya mengandalkan JS renderBookmarks, atau menggunakan @forelse untuk initial load --}}
                        @empty
                        @endforelse
                    </div>
    ...
    <script>
        // Ambil root URL storage dari Blade
        ...
        function renderBookmarks(type = "{{ request('type', 'all') }}", category = "{{ request('category', 'all') }}") {
            ...
            filteredBookmarks.forEach(bookmark => {
                ...
                if (!item) return; // Skip if bookmarkable is null (misalnya karena soft delete)
                
                const itemType = bookmark.bookmarkable_type.split('\\').pop();
                
                const route = {
                    'Artwork': '/galeri/' + item.id,
                    'Blog': '/blogs/' + item.id,
                ...
                // PERBAIKAN UTAMA: Tambahkan prefix storageUrl HANYA jika path gambar tidak full URL (memiliki '/')
                const rawImage = item.thumbnail || item.cover_image || item.image;
                const image = rawImage ? (rawImage.startsWith('http') ? rawImage : `${storageUrl}/${rawImage}`) : 'https://picsum.photos/600/400';
        ...
        renderBookmarks(initialType, initialCategory);
        renderCategories(initialType);
