<x-layout>
    ...
    <section class="pt-24 pb-12 bg-white">
        <div class="max-w-7xl mx-auto px-6 flex flex-col md:flex-row gap-6">
            <aside class="w-full md:w-64 bg-gray-50 border border-gray-200 rounded-xl p-6 sticky top-4 h-[calc(100vh-2rem)] overflow-y-auto">
                ...
                <button onclick="toggleNotifications()" class="flex items-center gap-2 text-gray-700 hover:text-yellow-400 transition py-2">
                    <i class="fas fa-bell"></i> Notifikasi <span id="unread-count" class="ml-1 text-xs bg-yellow-400 text-gray-900 rounded-full px-2 py-1">0</span>
                </button>
            ...
            <div class="flex-1">
                <div id="notification-bar" class="bg-gray-100 border border-gray-200 rounded-lg p-4 mb-6 cursor-pointer hover:shadow-lg hover:-translate-y-1 transition-all duration-300 hidden" onclick="dismissNotification()">
            ...
            <aside class="w-full md:w-48 bg-gray-50 border border-gray-200 rounded-xl p-6 sticky top-4 h-[calc(100vh-2rem)] overflow-y-auto">
                ...
                <div id="quick-notifications" class="space-y-2"></div>
            </aside>
        </div>
        ...
        <div id="notification-modal" class="fixed inset-y-0 right-0 w-full sm:w-96 bg-white shadow-xl transform translate-x-full transition-transform duration-300 z-50">
            <div class="p-6">
                <div class="flex items-center justify-between mb-4">
                    <h3 class="text-lg font-semibold text-gray-900 font-space-grotesk">Notifikasi</h3>
                    <button id="mark-all-read" class="text-sm text-gray-700 hover:text-yellow-400">Tandai semua dibaca</button>
                </div>
                <div class="flex gap-2 mb-4">
                    <button class="filter-btn px-4 py-2 bg-gray-100 text-sm text-gray-700 rounded-full hover:bg-gray-200 transition duration-300 active:bg-gray-900 active:text-white" data-filter="all">Semua</button>
    ...
    @push('scripts')
        <script>
            const routes = {
                komunitasShow: id => `/komunitas/${id}`,
                komunitasIndex: '/komunitas',
                forum: id => `/forum?community=${id}`,
            };
            ...
            function renderCommunities(filter = "all", searchQuery = "") {
                ...
                if (filteredCommunities.length === 0) {
                    communityList.innerHTML = `
                        <div class="text-center text-gray-600">
                            ...
                            <a href="${routes.komunitasIndex}" class="inline-block mt-4 px-6 py-3 bg-gray-900 text-white rounded-full font-semibold hover:bg-yellow-400 hover:text-gray-900 transition">Lihat Semua Komunitas</a>
                        </div>`;
                    return;
                }
                ...
                filteredCommunities.forEach(community => {
                    communityList.innerHTML += `
                        <a href="${routes.komunitasShow(community.id)}" class="block">
            ...
            function renderJoinedCommunities() {
                ...
                joinedCommunities.innerHTML = joined.length === 0
                    ...
                    : joined.map(c => `
                        <a href="${routes.forum(c.id)}" class="flex items-center gap-2 text-sm text-gray-700 hover:text-yellow-400 transition"><i class="fas fa-circle text-xs text-gray-900"></i> ${c.name}</a>`).join("");
            ...
            function viewNotification(notificationId) {
                ...
                renderNotifications(document.querySelector("#notification-modal .filter-btn.active")?.dataset.filter || "all");
                renderQuickNotifications();
                window.location.href = routes.komunitasShow(notification.communityId);
            }

            async function toggleJoin(id, button) {
                const community = communities.find(c => c.id === id);
                try {
                    const response = await fetch(`/komunitas/${id}/join`, {
                        method: 'POST',
                        headers: {
                    ...
                    if (!response.ok) throw new Error('Failed to toggle join');
                    ...
                    notificationBar.querySelector("p").textContent = data.message;
                    ...
                    alert('Gagal bergabung/keluar dari komunitas.');
                }
            }
            ...
            function viewActivity(activityId) {
                ...
                window.location.href = routes.komunitasShow(activity.communityId);
            }
