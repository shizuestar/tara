...
<head>
    ...
    <title>TARA – Detail Galeri</title>
...
<body class="relative overflow-x-hidden">
        ...
        <nav class="fixed top-0 left-0 right-0 z-50 glass-effect bg-white/80 backdrop-blur-md shadow-sm">
            <div class="max-w-7xl mx-auto px-6 py-4">
                <div class="flex items-center justify-between">
                    ...
                    <!-- Burger Icon Mobile -->
                    <button aria-label="af" id="burger-toggle" class="md:hidden focus:outline-none z-[60] relative">
                    ...
                    <div id="nav-menu" class="hidden md:flex md:flex-row flex-col md:items-center md:gap-8 absolute md:static top-full left-0 w-full md:w-auto bg-white md:bg-transparent shadow-md md:shadow-none px-6 py-6 md:p-0 z-50 transition-all duration-300">
                        ...
                        <a href="/komunitas.html" class="nav-link block text-gray-700 hover:text-black font-medium py-2">Komunitas</a>
            ...
            <section class="hero-wrapper px-4" id="gallery-hero">
                <div class="uploader-section">
                    ...
                    <div class="uploader-info">
                        <img id="gallery-uploader-avatar" src="https://i.pravatar.cc/60?img=22" alt="avatar" class="w-10 h-10 rounded-full object-cover" />
                        <div class="text-left leading-tight">
                            <div id="gallery-uploader-name" class="font-semibold text-[var(--tara-dark)] text-sm">Uploader Contoh</div>
                            <div id="gallery-posted-rel" class="text-xs text-gray-500">Diposting hari ini</div>
                        </div>
                    </div>
                    </a>
                    <button id="follow-btn" class="px-3 py-1 text-xs rounded-full bg-[var(--tara-accent)] text-black font-semibold hover:bg-yellow-300 transition">Follow</button>
                </div>
                <div class="flex flex-col items-center text-center gap-6">
                    ...
                    <!-- title -->
                    <h1 id="gallery-title" class="text-3xl md:text-4xl font-bold max-w-3xl leading-tight" style="font-family:'Space Grotesk',sans-serif;">Judul Galeri</h1>
                    ...
                    <div class="flex flex-wrap items-center justify-center gap-4 text-xs text-gray-500">
                        ...
                        <span id="gallery-category-meta" class="inline-flex items-center px-2 py-1 bg-gray-100 rounded-full text-gray-700"></span>
                    </div>
                </div>
            </section>

            <!-- Gallery Description -->
            <article id="gallery-body" class="gallery-content max-w-3xl mx-auto mt-16 px-4 prose prose-sm sm:prose-base lg:prose-lg prose-img:rounded-xl prose-headings:font-semibold prose-a:text-[var(--tara-dark)]">
        ...
        <div class="lg:col-span-4">
            ...
            <aside class="comment-panel" id="comment-panel" aria-labelledby="comment-panel-title">
                ...
                <div class="comment-form" id="comment-form-desktop">
                    <textarea id="new-comment-text" maxlength="300" placeholder="Tambahkan komentar... (Maks 300 karakter)" class="px-3 py-2 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-black"></textarea>
                    <div class="comment-char" id="comment-char-count">0/300</div>
                    <button id="submit-comment" class="mt-2 w-full px-4 py-2 bg-black text-white text-sm rounded-md hover:bg-gray-800 transition">Kirim</button>
                </div>
            </aside>
        </div>
    </main>
    ...
    <!-- Mobile Comment Toggle -->
    <button id="mobile-comment-toggle"><i class="fas fa-comments"></i><span>Komentar</span><span class="count" id="mobile-comment-count">0</span></button>
    ...
    <div id="comment-drawer" class="comment-drawer" role="dialog" aria-modal="true" aria-labelledby="comment-drawer-title">
        ...
        <div class="comment-drawer-footer">
            <textarea id="new-comment-mobile" maxlength="300" placeholder="Tambahkan komentar..." class="px-3 py-2 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-black"></textarea>
            <div class="comment-char" id="comment-char-count-mobile">0/300</div>
            <button id="submit-comment-mobile" class="mt-2 w-full px-4 py-2 bg-black text-white text-sm rounded-md hover:bg-gray-800 transition">Kirim</button>
        </div>
    </div>
    ...
    <script>
        ...
        const demoGalleryItems = [
            {
                ...
                body: [
                    { t: "p", c: "Karya ini menangkap keindahan senja dengan permainan cahaya dan bayangan yang dramatis." },
                    { t: "img", src: "https://picsum.photos/seed/gallery1b/1100/600", alt: "Detail senja" },
                    { t: "p", c: "Fotografi ini menggunakan lensa 50mm dengan aperture f/2.8 untuk efek bokeh." }
                ]
            },
            {
                id: 2,
                ...
                body: [
                    { t: "p", c: "Karya ini dibuat menggunakan tablet grafis dan software Procreate." },
                    { t: "img", src: "https://picsum.photos/seed/gallery2b/900/500", alt: "Detail ilustrasi" },
                    { t: "p", c: "Inspirasi berasal dari mitologi lokal dan cerita rakyat." }
                ]
            },
            {
                id: 3,
                ...
                body: [
                    { t: "p", c: "Desain ini mengutamakan pengalaman pengguna dengan navigasi intuitif." },
                    { t: "img", src: "https://picsum.photos/seed/gallery3b/1100/600", alt: "Prototipe UI" }
                ]
            },
            {
                id: 7,
                ...
                body: [
                    { t: "p", c: "Gambar TV tabung yang tertanam di bukit pasir ini simbol transisi media lama ke dunia digital." },
                    { t: "img", src: "https://picsum.photos/seed/tvpasir-b/1100/600", alt: "TV di pasir" },
                    { t: "p", c: "Menggunakan lensa wide 24mm, aperture f/4, dan cahaya alami sore." }
                ]
            }
        ];
        ...
        qs('#gallery-category-meta').textContent = galleryData.category.charAt(0).toUpperCase() + galleryData.category.slice(1);
        qs('#gallery-date-meta').innerHTML = `<i class="fas fa-calendar-alt mr-1"></i>${fmtDateISOtoDisplay(galleryData.date)}`;
        qs('#gallery-posted-rel').textContent = 'Diposting ' + relDate(galleryData.date);
        qs('#gallery-views-meta').innerHTML = `<i class="fas fa-eye mr-1"></i>${galleryData.views.toLocaleString('id-ID')} Dilihat`;

        // tags
        const tagWrap = qs('#gallery-tags');
        (galleryData.tags || []).forEach(t => {
            const btn = document.createElement('button'); btn.textContent = '#' + t; btn.addEventListener('click', () => { window.location.href = `/galeri?tag=${encodeURIComponent(t)}`; }); tagWrap.appendChild(btn);
        });
        ...
        galleryData.body.forEach(block => {
            let el;
            switch (block.t) {
                case 'p': el = document.createElement('p'); el.textContent = block.c; break;
                case 'img': el = document.createElement('img'); el.src = block.src; el.alt = block.alt || ''; break;
                default: return;
            }
        ...
        const demoComments = [
            {
                id: 101, user: "Pengguna Anonim", handle: "@anon", avatar: "https://i.pravatar.cc/60?img=5", text: `Ini Karya Galeri ID ${galleryId}`, dateISO: "2025-07-19T23:46:25", likes: 0, replies: [
                    { id: 102, user: "Pengguna Lain", handle: "@pengguna", avatar: "https://i.pravatar.cc/60?img=14", text: "Keren banget konsepnya!", dateISO: "2025-07-19T23:46:30", likes: 0 }
                ]
            },
            { id: 103, user: "Rizky", handle: "@rizky", avatar: "https://i.pravatar.cc/60?img=18", text: "Inspiratif! Apa teknik yang dipakai?", dateISO: "2025-07-20T08:12:10", likes: 2, replies: [] }
        ];
        ...
        function fmtDateTime(iso) {
            const d = new Date(iso); return d.toLocaleString('id-ID', { day: 'numeric', month: 'numeric', year: 'numeric', hour: '2-digit', minute: '2-digit', second: '2-digit' });
        }
        ...
        function attachCommentEvents(root, isMobile) {
            root.querySelectorAll('[data-action="menu"]').forEach(btn => {
                btn.addEventListener('click', e => {
                    const pop = e.currentTarget.nextElementSibling; pop.classList.toggle('open');
                });
            });
            root.querySelectorAll('[data-action="like"]').forEach(btn => {
                btn.addEventListener('click', e => {
                    ...
                    if (reply) { reply.likes++; } else if (comment) { comment.likes++; }
                    e.currentTarget.classList.add('active');
                    e.currentTarget.querySelector('span').textContent = (reply ? reply.likes : comment.likes);
                    syncBadges();
                });
            });
            root.querySelectorAll('[data-action="toggle"]').forEach(btn => {
                btn.addEventListener('click', e => {
                    ...
                    if (!rep) return; rep.classList.toggle('hidden');
                    e.currentTarget.textContent = rep.classList.contains('hidden') ? `Lihat balasan (${rep.children.length})` : 'Sembunyikan Balas';
                });
            });
            ...
            root.querySelectorAll('[data-action="cancel-reply"]').forEach(btn => {
                btn.addEventListener('click', e => {
                    ...
                    form.classList.add('hidden'); form.querySelector('textarea').value = '';
                });
            });
            root.querySelectorAll('[data-action="submit-reply"]').forEach(btn => {
                btn.addEventListener('click', e => {
                    ...
                    addReply(id, text);
                    renderCommentsDesktop(); renderCommentsMobile(); updateCommentCounts();
                });
            });
            root.querySelectorAll('[data-action="report"]').forEach(btn => btn.addEventListener('click', () => alert('Dilaporkan (demo).')));
            root.querySelectorAll('[data-action="mute"]').forEach(btn => btn.addEventListener('click', () => alert('Pengguna dibisukan (demo).')));
        }

        function findCommentById(id) {
            for (const c of demoComments) {
                if (c.id === id) return { comment: c, reply: null };
                for (const r of c.replies || []) { if (r.id === id) return { comment: c, reply: r }; }
            }
            return { comment: null, reply: null };
        }
        function addComment(text) {
            const now = new Date();
            demoComments.unshift({ id: Date.now(), user: "Anda", handle: "@anda", avatar: "https://i.pravatar.cc/60?u=self", text, dateISO: now.toISOString(), likes: 0, replies: [] });
        }
        function addReply(parentId, text) {
            const now = new Date();
            const { comment } = findCommentById(parentId); if (!comment) return;
            comment.replies = comment.replies || [];
            comment.replies.push({ id: Date.now(), user: "Anda", handle: "@anda", avatar: "https://i.pravatar.cc/60?u=self", text, dateISO: now.toISOString(), likes: 0 });
        }
        function syncBadges() {
            const likeCount = Number(localStorage.getItem('tara-gallery-like-' + galleryData.id) || 0);
            const badge = qs('#float-like-badge');
            if (likeCount > 0) { badge.textContent = likeCount; badge.classList.remove('hidden'); } else { badge.classList.add('hidden'); }
        }
        ...
        newCommentText?.addEventListener('input', () => { charCount.textContent = `${newCommentText.value.length}/300`; });
        qs('#submit-comment')?.addEventListener('click', () => {
            const text = newCommentText.value.trim(); if (!text) return;
            addComment(text); newCommentText.value = ''; charCount.textContent = '0/300'; renderCommentsDesktop(); renderCommentsMobile(); updateCommentCounts();
        });
        const newCommentMobile = qs('#new-comment-mobile');
        const charCountMobile = qs('#comment-char-count-mobile');
        newCommentMobile?.addEventListener('input', () => { charCountMobile.textContent = `${newCommentMobile.value.length}/300`; });
        qs('#submit-comment-mobile')?.addEventListener('click', () => {
            const text = newCommentMobile.value.trim(); if (!text) return;
            addComment(text); newCommentMobile.value = ''; charCountMobile.textContent = '0/300'; renderCommentsDesktop(); renderCommentsMobile(); updateCommentCounts();
        });
        ...
        qs('#mobile-comment-toggle').addEventListener('click', () => { drawer.classList.add('open'); });
        qs('#comment-drawer-close').addEventListener('click', () => { drawer.classList.remove('open'); });
        ...
        floatLikeBtn.addEventListener('click', () => {
            const key = 'tara-gallery-like-' + galleryData.id; let count = Number(localStorage.getItem(key) || 0); count++; localStorage.setItem(key, count);
            syncBadges();
            anime({ targets: floatLikeBtn, scale: [1, 1.2, 1], duration: 300, easing: 'easeOutQuad' });
        });
        ...
        qs('#share-instagram').href = `https://www.instagram.com/?url=${shareUrl}`; // Instagram doesn't support direct sharing with title
        ...
        gsap.from('#gallery-hero-image-wrapper', { opacity: 0, y: 40, duration: 1, ease: 'power3.out' });
